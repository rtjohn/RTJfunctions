function AFQ_exportDataMINDtotals(data1, data2, filename, property, sub_ids)
...
% AFQ_exportDataMIND(patient_data, control_data, [AFQdata '/results'], 'fa', sub_ids);
%
...
for jj = 1:length(data1)
    %Create a column that contains an average fiber score for each subject
    n = (data1(jj).(property));
    avg = mean(n,2);
...
% write out data for each fiber group for the first data array(data2) as a separate file
...
% Stack the data1 and data 2 columns vertically
...
%Write out the spreadsheet containing both data1 and data2 mean fiber
