...
cd ('/Volumes/data/data/APP/Projects/Amygdala/ROIFA/temporarystorage/LeftCleaned');
theseFiles = dir('/Volumes/data/data/APP/Projects/Amygdala/ROIFA/temporarystorage/LeftCleaned/*.mat');
...
delete('/Volumes/data/data/APP/Projects/Amygdala/ROIFA/temporarystorage/LeftCleaned/L*.mat');
%The above grabs files from LeftCleaned and renames them to remove the 'L' at the start of
%the file name.  It then deletes the files that start with 'L'.
...
for k = 1:length(theseFiles);
    ...
    fname = n;
    f = fullfile('/Volumes/data/data/APP/Projects/Amygdala/Amy_DTI', n);
    cd(f);
    if(~exist('ROIs','dir'));
        mkdir ROIs;
    end
    ...
    copyfile(dtifile, '/Volumes/data/data/APP/Projects/Amygdala/ROIFA/FAFiles');
...
%Now we just repeat the same steps for the Right ROIs
clear all
cd ('/Volumes/data/data/APP/Projects/Amygdala/ROIFA/temporarystorage/RightCleaned');
theseFiles = dir('/Volumes/data/data/APP/Projects/Amygdala/ROIFA/temporarystorage/RightCleaned/*.mat');
smoothKernel = 3;
...
delete('/Volumes/data/data/APP/Projects/Amygdala/ROIFA/temporarystorage/RightCleaned/R*.mat');
theseFiles = dir('/Volumes/data/data/APP/Projects/Amygdala/ROIFA/temporarystorage/RightCleaned/*.mat');
...
%Below is  cleaning out the Binary ROIs folder by moving the .nii
%ROIs to their correct subject folder
cd ('/Volumes/data/data/APP/Projects/Amygdala/ROIFA/BinaryROIs');
sortFiles = dir('/Volumes/data/data/APP/Projects/Amygdala/ROIFA/BinaryROIs/*.nii');

for k = 1:length(sortFiles);
    filename = sortFiles(k).name;
    [p,n,e] = fileparts(filename);
    fname = n(2:end);
    f = fullfile('/Volumes/data/data/APP/Projects/Amygdala/Amy_DTI', fname, 'ROIs');
    movefile(filename, f);
end    
    
fprintf('Cleaned ROIs have been moved to subjects DTI folder.  FA text files have been exported to the FAFiles folder')
